...
public class AgentServer{
	...
	public  SelectorLoop connectEvent;
	public  SelectorLoop readEvent;
	public boolean isReadBellRunning=false;
	private int port=10001;
	//private Charset charset=Charset.forName("GBK");//返回一个字符类型对象
	public AgentServer() throws IOException{
	...
	public void service(BizCommonService bizCommonService) throws IOException{
		 //连接线程
		 connectEvent = new SelectorLoop(bizCommonService);
		 //读取线程
		 readEvent= new SelectorLoop(bizCommonService);
		try {
			//用给定的选择器注册channel，并返回一个key,只监听连接事件
			serverSocketChannel.register(connectEvent.getSelector(), SelectionKey.OP_ACCEPT);
		} catch (ClosedChannelException e) {
			e.printStackTrace();
		}
		new Thread(connectEvent).start();
	}
	public class SelectorLoop implements Runnable{
		...
		public SelectorLoop(BizCommonService bizCommonService) throws IOException {
			this.selector = Selector.open();
			...
			try {
				while(selector.select()>0){
					...
					while(it.hasNext()){
						...
						it.remove();//删除集合中的key
						this.manage(key,bizCommonService);
				  }
		...
		public void manage(SelectionKey key,BizCommonService bizCommonService){
			try{
				if(key.isAcceptable()){//是否可以接收客户端的socket连接
					...
					//监听读写事件
					socketChannel.register(readEvent.getSelector(),SelectionKey.OP_READ|SelectionKey.OP_WRITE,buffer);//注册事件,Selector会监控事件是否发生
					synchronized(AgentServer.this) {
						if (!AgentServer.this.isReadBellRunning) {
							AgentServer.this.isReadBellRunning = true;
							new Thread(readEvent).start();
						}
					}
				}
	...
	public static void main(String[] args) throws IOException {
		...
		try{
			...
			new AgentServer().service(bizCommonService);  
		}catch(Exception e){
